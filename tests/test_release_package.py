...
OUTPUT = ROOT / "dist" / "Skin_and_Bones_Forge_v2.2.2.zip"
...
class ReleasePackageTests(unittest.TestCase):
    ...
    def test_release_zip_is_deterministic_and_installable(self):
        ...
        with zipfile.ZipFile(OUTPUT) as archive:
            ...
            self.assertIn("skin_and_bones_forge/weapon_projection.py", names)
            self.assertIn(
                "skin_and_bones_forge/weapon_projection_full_strength.py", names
            )
            self.assertIn(
                "skin_and_bones_forge/baking/texture_repair.py", names
            )
            self.assertIn(
                "skin_and_bones_forge/baking/repair_service.py", names
            )
            self.assertIn(
            ...
            self.assertIn(
                "skin_and_bones_forge/assets/"
                "canonical_humanoid_yplus_v1.blend",
                names,
            )
            self.assertIn(
                "skin_and_bones_forge/assets/"
                "canonical_humanoid_yplus_v1.contract.json",
                names,
            )
            self.assertTrue(
